...
import './styleV2.scss';
import 'bootstrap';
import {DemoTileOrganizer} from './DemoTileOrganizer';
import {TestSound} from './SoundControl';
import {
  AsyncScheduler,
  // AudioVideoFacade,
  AudioVideoObserver,
...
export class DemoMeetingApp implements AudioVideoObserver, DeviceChangeObserver, ContentShareObserver {
  ...
  initEventListeners(): void {
    window.addEventListener('resize', () => {
      this.tileOrganizer.layoutVideoTiles();
    });
    ...
    document.getElementById('form-authenticate').addEventListener('submit', e => {
      ...
      new AsyncScheduler().start(
        async (): Promise<void> => {
          ...
          (document.getElementById(
            'mobile-attendee-id'
          ) as HTMLSpanElement).innerText = `Attendee ID: ${this.tileOrganizer.meetingSession.configuration.credentials.attendeeId}`;
          (document.getElementById(
            'desktop-attendee-id'
          ) as HTMLSpanElement).innerText = `Attendee ID: ${this.tileOrganizer.meetingSession.configuration.credentials.attendeeId}`;
    ...
    videoInputQuality.addEventListener('change', async (_ev: Event) => {
      ...
      switch (videoInputQuality.value) {
        case '360p':
          this.tileOrganizer.audioVideo.chooseVideoInputQuality(640, 360, 15, 600);
          break;
        case '540p':
          this.tileOrganizer.audioVideo.chooseVideoInputQuality(960, 540, 15, 1400);
          break;
        case '720p':
          this.tileOrganizer.audioVideo.chooseVideoInputQuality(1280, 720, 15, 1400);
          break;
      }
      try {
        await this.openVideoInputFromSelection(videoInput.value, true);
    ...
    document.getElementById('form-devices').addEventListener('submit', e => {
      ...
      new AsyncScheduler().start(async () => {
        try {
          ...
          await this.join();
          this.tileOrganizer.audioVideo.stopVideoPreviewForVideoInput(document.getElementById(
            'video-preview'
          ) as HTMLVideoElement);
          this.tileOrganizer.audioVideo.chooseVideoInputDevice(null);
    ...
    buttonMute.addEventListener('mousedown', _e => {
      if (this.toggleButton('button-microphone')) {
        this.tileOrganizer.audioVideo.realtimeUnmuteLocalAudio();
      } else {
        this.tileOrganizer.audioVideo.realtimeMuteLocalAudio();
      }
    });
    ...
    buttonVideo.addEventListener('click', _e => {
      new AsyncScheduler().start(async () => {
        if (this.toggleButton('button-camera') && this.tileOrganizer.canStartLocalVideo) {
          try {
            ...
            this.tileOrganizer.audioVideo.startLocalVideoTile();
          } catch (err) {
            this.log('no video input device selected');
          }
        } else {
          this.tileOrganizer.audioVideo.stopLocalVideoTile();
          this.tileOrganizer.hideTile(DemoTileOrganizer.MAX_TILES);
        }
      });
    });
// #################################################################################################
    ...
    buttonPauseContentShare.addEventListener('click', _e => {
      ...
      new AsyncScheduler().start(async () => {
        if (this.toggleButton('button-pause-content-share')) {
          this.tileOrganizer.audioVideo.pauseContentShare();
        } else {
          this.tileOrganizer.audioVideo.unpauseContentShare();
        }
      });
    });
...
// ###############################################################################################
    ...
    buttonSpeaker.addEventListener('click', _e => {
      new AsyncScheduler().start(async () => {
        if (this.toggleButton('button-speaker')) {
          this.tileOrganizer.audioVideo.bindAudioElement(document.getElementById(
            'meeting-audio'
          ) as HTMLAudioElement);
        } else {
          this.tileOrganizer.audioVideo.unbindAudioElement();
        }
      });
    });
    ...
    const sendMessage = () => {
      new AsyncScheduler().start(() => {
        ...
        textArea.value = '';
        this.tileOrganizer.audioVideo.realtimeSendDataMessage(DemoMeetingApp.DATA_MESSAGE_TOPIC, textToSend, DemoMeetingApp.DATA_MESSAGE_LIFETIME_MS);
        // echo the message to the handler
        this.dataMessageHandler(new DataMessage(
          Date.now(),
          DemoMeetingApp.DATA_MESSAGE_TOPIC,
          new TextEncoder().encode(textToSend),
          this.tileOrganizer.meetingSession.configuration.credentials.attendeeId,
          this.tileOrganizer.meetingSession.configuration.credentials.externalUserId
        ));
      });
    };
...
// @#))))))))))((((((((((((((((((((((((((((((((((((((((((((((((((((((((()))))))))))))))))))))))))))))))))))))))))))))))))))))))))
  ...
  async initializeMeetingSession(configuration: MeetingSessionConfiguration): Promise<void> {
    ...
    this.tileOrganizer.meetingSession = new DefaultMeetingSession(configuration, logger, deviceController);
    this.tileOrganizer.audioVideo = this.tileOrganizer.meetingSession.audioVideo;

    this.tileOrganizer.audioVideo.addDeviceChangeObserver(this);
    ...
    this.tileOrganizer.audioVideo.addObserver(this);
    this.tileOrganizer.audioVideo.addContentShareObserver(this);
  ...
  async join(): Promise<void> {
    ...
    await this.openAudioInputFromSelection();
    await this.openAudioOutputFromSelection();
    this.tileOrganizer.audioVideo.start();
  }

  leave(): void {
    this.tileOrganizer.audioVideo.stop();
    this.tileOrganizer.roster = {};
  }
  ...
  setupMuteHandler(): void {
    ...
    this.tileOrganizer.audioVideo.realtimeSubscribeToMuteAndUnmuteLocalAudio(handler);
    const isMuted = this.tileOrganizer.audioVideo.realtimeIsLocalAudioMuted();
    handler(isMuted);
  }
  ...
  setupCanUnmuteHandler(): void {
    ...
    this.tileOrganizer.audioVideo.realtimeSubscribeToSetCanUnmuteLocalAudio(handler);
    handler(this.tileOrganizer.audioVideo.realtimeCanUnmuteLocalAudio());
  }

  updateRoster(): void {
    const roster = document.getElementById('roster');
    const newRosterCount = Object.keys(this.tileOrganizer.roster).length;
    ...
    let i = 0;
    for (const attendeeId in this.tileOrganizer.roster) {
      ...
      if (this.tileOrganizer.roster[attendeeId].signalStrength < 1) {
        statusClass += 'badge-warning';
      } else if (this.tileOrganizer.roster[attendeeId].signalStrength === 0) {
        statusClass += 'badge-danger';
      } else if (this.tileOrganizer.roster[attendeeId].muted) {
        statusText = 'MUTED';
        statusClass += 'badge-secondary';
      } else if (this.tileOrganizer.roster[attendeeId].active) {
        statusText = 'SPEAKING';
        statusClass += 'badge-success';
      } else if (this.tileOrganizer.roster[attendeeId].volume > 0) {
        statusClass += 'badge-success';
      }
      this.updateProperty(spanName, 'innerText', this.tileOrganizer.roster[attendeeId].name);
  ...
  setupSubscribeToAttendeeIdPresenceHandler(): void {
    const handler = (attendeeId: string, present: boolean, externalUserId: string): void => {
      ...
      const isSelfAttendee = new DefaultModality(attendeeId).base() === this.tileOrganizer.meetingSession.configuration.credentials.attendeeId;
      if (!present) {
        delete this.tileOrganizer.roster[attendeeId];
        this.updateRoster();
        return;
      }
      ...
      if (!this.tileOrganizer.roster[attendeeId]) {
        this.tileOrganizer.roster[attendeeId] = {
          name: (externalUserId.split('#').slice(-1)[0]) + (isContentAttendee ? ' «Content»' : ''),
        };
      }
      this.tileOrganizer.audioVideo.realtimeSubscribeToVolumeIndicator(
        attendeeId,
        async (
          attendeeId: string,
          volume: number | null,
          ...
          if (volume !== null) {
            this.tileOrganizer.roster[attendeeId].volume = Math.round(volume * 100);
          }
          if (muted !== null) {
            this.tileOrganizer.roster[attendeeId].muted = muted;
          }
          if (signalStrength !== null) {
            this.tileOrganizer.roster[attendeeId].signalStrength = Math.round(signalStrength * 100);
          }
          this.updateRoster();
        }
      );
    };
    this.tileOrganizer.audioVideo.realtimeSubscribeToAttendeeIdPresence(handler);
    const activeSpeakerHandler = (attendeeIds: string[]): void => {
      for (const attendeeId in this.tileOrganizer.roster) {
        this.tileOrganizer.roster[attendeeId].active = false;
      }
      for (const attendeeId of attendeeIds) {
        if (this.tileOrganizer.roster[attendeeId]) {
          this.tileOrganizer.roster[attendeeId].active = true;
          break; // only show the most active speaker
        }
      }
      this.tileOrganizer.layoutVideoTiles();
    };
    this.tileOrganizer.audioVideo.subscribeToActiveSpeakerDetector(
      new DefaultActiveSpeakerPolicy(),
      activeSpeakerHandler,
      (scores: {[attendeeId:string]: number}) => {
        for (const attendeeId in scores) {
          if (this.tileOrganizer.roster[attendeeId]) {
            this.tileOrganizer.roster[attendeeId].score = scores[attendeeId];
          }
        }
        this.updateRoster();
      },
  ...
  setupDataMessage(): void {
    this.tileOrganizer.audioVideo.realtimeSubscribeToReceiveDataMessage(DemoMeetingApp.DATA_MESSAGE_TOPIC, (dataMessage: DataMessage) => {
  ...
  setupDeviceLabelTrigger(): void {
    ...
    this.tileOrganizer.audioVideo.setDeviceLabelTrigger(
  ...
  async populateAudioInputList(): Promise<void> {
    ...
    this.populateDeviceList(
      'audio-input',
      genericName,
      await this.tileOrganizer.audioVideo.listAudioInputDevices(),
      additionalDevices
    );
    this.populateInMeetingDeviceList(
      ...
      additionalDevices,
      async (name: string) => {
        await this.tileOrganizer.audioVideo.chooseAudioInputDevice(this.audioInputSelectionToDevice(name));
      }
    );
  }
  ...
  async populateVideoInputList(): Promise<void> {
    ...
    this.populateDeviceList(
      'video-input',
      genericName,
      await this.tileOrganizer.audioVideo.listVideoInputDevices(),
      additionalDevices
    );
    this.populateInMeetingDeviceList(
    ...
    const cameras = await this.tileOrganizer.audioVideo.listVideoInputDevices();
  ...
  async populateAudioOutputList(): Promise<void> {
    ...
    this.populateDeviceList(
      'audio-output',
      genericName,
      await this.tileOrganizer.audioVideo.listAudioOutputDevices(),
      additionalDevices
    );
    this.populateInMeetingDeviceList(
      ...
      additionalDevices,
      async (name: string) => {
        await this.tileOrganizer.audioVideo.chooseAudioOutputDevice(name);
      }
    );
  }
  ...
  async openAudioInputFromSelection(): Promise<void> {
    const audioInput = document.getElementById('audio-input') as HTMLSelectElement;
    await this.tileOrganizer.audioVideo.chooseAudioInputDevice(
  ...
  startAudioPreview(): void {
    ...
    const analyserNode = this.tileOrganizer.audioVideo.createAnalyserNodeForAudioInput();
    if (!analyserNode) {
      return;
    }
  ...
  async openAudioOutputFromSelection(): Promise<void> {
    const audioOutput = document.getElementById('audio-output') as HTMLSelectElement;
    await this.tileOrganizer.audioVideo.chooseAudioOutputDevice(audioOutput.value);
    const audioMix = document.getElementById('meeting-audio') as HTMLAudioElement;
    await this.tileOrganizer.audioVideo.bindAudioElement(audioMix);
  }
  ...
  async openVideoInputFromSelection(selection: string | null, showPreview: boolean): Promise<void> {
    ...
    if (device === null) {
      ...
      await this.tileOrganizer.audioVideo.chooseVideoInputDevice(device);
      throw new Error('no video device selected');
    }
    await this.tileOrganizer.audioVideo.chooseVideoInputDevice(device);
    if (showPreview) {
      this.tileOrganizer.audioVideo.startVideoPreviewForVideoInput(document.getElementById(
        'video-preview'
      ) as HTMLVideoElement);
    }
  }
...
// #############################################################################################
  ...
  private async contentShareStart(videoUrl?: string): Promise<void> {
    ...
    switch (this.contentShareType) {
      case ContentShareType.ScreenCapture:
        this.tileOrganizer.audioVideo.startContentShareFromScreenCapture();
        break;
      case ContentShareType.VideoFile:
        ...
        this.tileOrganizer.audioVideo.startContentShare(mediaStream);
        break;
    }
  }
  ...
  private async contentShareStop(): Promise<void> {
    ...
    this.toggleButton('button-content-share');
    this.tileOrganizer.audioVideo.stopContentShare();
